...
class TestUsers(ApiTestCase):
    ...
    def test_returns_200(self):
        res = self.app.get('/{}users/'.format(API_PREFIX))  # TODO | this is '//v2/' but should be '/v2' - but
                                                            # TODO | all tests passed
        assert_equal(res.status_code, 200)

    def test_find_user_in_users(self):
        url = "/{}users/".format(API_PREFIX)
    ...
    def test_find_multiple_in_users(self):
        url = "/{}users/?filter[fullname]=fred".format(API_PREFIX)
    ...
    def test_find_single_user_in_users(self):
        url = "/{}users/?filter[fullname]=my".format(API_PREFIX)
    ...
    def test_find_no_user_in_users(self):
        url = "/{}users/?filter[fullname]=NotMyMom".format(API_PREFIX)
        res = self.app.get(url)
        user_json = res.json['data']
        ids = [each['id'] for each in user_json]
...
class TestUserDetail(ApiTestCase):
    ...
    def test_gets_200(self):
        url = "/{}users/{}/".format(API_PREFIX, self.user_one._id)
        res = self.app.get(url)
        assert_equal(res.status_code, 200)
    ...
    def test_get_incorrect_pk_user_logged_in(self):
        url = "/{}users/{}/".format(API_PREFIX, self.user_two._id)
        res = self.app.get(url)
        user_json = res.json['data']
...
class TestUserNodes(ApiTestCase):
    ...
    def test_authorized_in_gets_200(self):
        url = "/{}users/{}/nodes/".format(API_PREFIX, self.user_one._id)
        res = self.app.get(url, auth=self.auth_one)
        assert_equal(res.status_code, 200)
    ...
    def test_get_projects_logged_in_as_different_user(self):
        url = "/{}users/{}/nodes/".format(API_PREFIX, self.user_two._id)
        res = self.app.get(url, auth=self.auth_one)
        node_json = res.json['data']
